...
// @name         CWS 7dl
// @version      1.0
// @description  Встроенная карта 7ДЛ с таймерами действий
// @author       ReiReiRei
...
(function (window, document, $) {
    'use strict';
    function getSettings(key, def = null) { //Получение настроек
    ...
    if (/^https:\/\/\w*\.?catwar.(su|net)\/$/.test(window.location.href)) {
        ...
        if ($('.other_cats_list').length) {
            ...
            $('body').prepend('<span style="background: white; color: black;"><span id="cws_localTimer">' + getTimeStr() + '</span> | <span id="cws_actionTimer">' + getActionTimeStr() + '</span></span>');
        }
    ...
    if (/^https:\/\/\w*\.?catwar.(su|net)\/cw3(?!(\/kns|\/jagd))/.test(window.location.href)) {
        ...
        actionsObserver.observe(document.getElementById('tr_actions'), {
            childList: true,
            subtree: true,
            characterData: true,
        ...
        html += '</table></div>';
        html += '<div><small>Зелёное - к МВ (МП), синее - к ОВ (ОП)</small></div><hr>';
        ...
        $('#cws_7dl_lab_choice').change(function(e) {
            if (!ready) return;
            let new_choice = $('#cws_7dl_lab_choice').find(":selected").val();
        ...
        function switchLoc(newLoc) {
            ...
            if (newLoc != loc_choice) {
                ...
                let count = Object.keys(map[lab]).length / 2;
